...
|---|---|---|
| `stale_dot` | V1 | счётчик не больше последнего принятого (и такого dot в журнале нет — иначе это повтор, `ack`), **или** dot несёт актора, не заявленного этим соединением в `hello` (T-010: `owner(a) = u`) |
