...
| `ack` | `dot`, `seq` | своя операция принята (или была принята раньше — повтор: V1 для `create`/`write`/`vote`, V7 для `unvote`, ADR-0008) |
| `reject` | `dot`, `reason`, `message` | своя операция отклонена (REQ-024) |
| `op` | `seq`, `delta` | чужая принятая операция, в проекции получателя |
| `meta` | `meta: BoardMeta`, `role?` | изменились фаза, таймер или авторы; `role` — только если у получателя изменилась собственная роль (`grantFacilitator`) |
...
|---|---|---|
| `stale_dot` | V1 | только для `create`/`write`/`vote`: счётчик не больше последнего принятого (и такого dot в журнале нет — иначе это повтор, `ack`), **или** dot несёт актора, не заявленного этим соединением в `hello` (T-010: `owner(a) = u`). Не применяется к `unvote` (ADR-0008) — см. `not_own_vote` |
| `invalid_shape` | V2 | не одна операция, не одна сущность (записи/создание на разные `entity`), значение вне домена, лимиты § 1 |
...
| `not_own_vote` | V7 | `unvote`: голос не существует, либо принадлежит другому `voterToken`. Повтор уже отозванного **этого же** голоса (в т.ч. отозванного `resetVotes`) — не ошибка, `ack` с `seq` той записи журнала, что его отозвала (ADR-0008) |
...
**Команды метаданных (T-030).** Не CRDT (`consistency-model.md` § 3.2), обрабатываются сервером в очереди доски; ответ — `commandResult` автору.
- `grantFacilitator {guestId}` (REQ-003 кр. 2–3): только `owner`, иначе `forbidden`; `guestId` не участник доски — `unknown_target`. Успех: роль цели становится `facilitator` (повтор для уже фасилитатора — `ok`, без изменений); каждому подключению цели уходит `meta` с полем `role: "facilitator"`.
- `startTimer {seconds}` (REQ-019 кр. 2): `owner`/`facilitator`, иначе `forbidden`; фаза не `discuss` — `wrong_phase`. `timer.endsAt = now + seconds` (время — порт сервера, § 7 ADR-0009); уже идущий таймер заменяется. Всем подписчикам — `meta`. Таймер только рекомендательный: по истечении сервер ничего не делает.
- `stopTimer`: те же роли; сбрасывает таймер (без таймера — `ok`, ничего не меняется), всем подписчикам — `meta`.
- `setPhase` на любую фазу, кроме `discuss`, сбрасывает таймер (в том же `meta`, что несёт новую фазу). Таймер переживает перезапуск сервера (REQ-027).

**Порядок обработки (ADR-0009).** Сервер обрабатывает сообщения одного соединения строго в порядке прихода, включая `hello`, и в общей очереди доски. `op`, пришедший сразу за `hello`, обрабатывается после `welcome`. Ответ `ack`/`reject` относится к **первой** ещё не подтверждённой дельте клиента с этим `dot` в порядке отправки: у `unvote` тот же `dot`, что у отзываемого `vote`. Клиент применяет `op`, пришедший раньше `welcome`, а не отбрасывает его. После `reject` клиент удаляет или пересобирает зависимые дельты своей очереди (ADR-0010). Ответы сервера на их прежние копии, уже отправленные, не совпадают по `dot` ни с одной дельтой очереди и игнорируются.

**Переподключение (REQ-023).** Клиент открывает соединение, шлёт `hello` с последним известным `seq` (максимум среди полученных: `seq` глобален по доскам и у клиента идёт с пропусками), **после `welcome`** отправляет очередь **в исходном порядке**. Повтор уже принятой операции даёт `ack` с исходным `seq` без повторного применения — по `(actor, counter)` для `create`/`write`/`vote` (V1), по паре `(dot отзываемого голоса, target)` в журнале для `unvote` (V7, ADR-0008: `unvote` не несёт собственного dot операции, привязки к актору соединения нет).
...
| `GET /api/boards/:boardId/members` | → `200 { members: [{ guestId, displayName, role }] }` (порядок — по времени входа, владелец первым); `404`, если доски нет или у `guestId` нет записи в `members` (как у `GET /api/boards/:boardId`). Доступ: `owner`/`facilitator` — всегда (выбор цели `grantFacilitator`); остальные роли, включая `viewer`, — только после `reveal` (фаза не `collect`), до этого `403 forbidden` (имена участников нужны, чтобы показать ответственного за action item, REQ-018; до `reveal` они скрыты вместе с авторством). Обновлено в T-019 | любой участник, см. доступ | REQ-003, REQ-018 |
