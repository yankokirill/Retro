...
| `GET /api/boards/:boardId/members` | → `200 { members: [{ guestId, displayName, role }] }` (порядок — по времени входа, владелец первым); `403 forbidden` роли `participant`/`viewer`; `404`, если доски нет или у `guestId` нет записи в `members` (как у `GET /api/boards/:boardId`). Нужен интерфейсу `owner` для выбора цели `grantFacilitator` (клиент не знает других участников: `authors` — только имена и только после `reveal`) | owner, facilitator | REQ-003 |
