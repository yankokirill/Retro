...
| S7 | причина ∈ одного разрешённого множества (ВС-7/ВС-8 — сопоставленный/устаревший больше не разделены по причинам, см. ВС-8: дубликаты стирают различие); `wasPending` (остался ли `dot` в `P` клиента к моменту доставки) — только для сводки/покрытия SIM-11, не для решения; dot отсутствует в журнале (обработка сообщения сервером не добавила строк); в покое — отсутствует в `confirmed` и `pending` автора |
