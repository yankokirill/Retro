...
- Журнал — массив строк, индексы `(actor,counter) → seq` и `"dot|target" → seq` для `unvote` (первое вхождение), `actorClock` — поддерживаемые максимумы.
- `currentState` — поддерживаемое инкрементально состояние (`merge` при `appendOp`), `uptoSeq` — последний `seq`.
- `transaction` — буфер записей: применяется в конце `fn`; при `failNextTransaction(k)` бросает после k-й записи в буфер, ничего не применив.
- Все выборки возвращают копии: мутация результата вызывающим не должна менять хранилище (контрактный тест).
- `seqGap` — единственный источник «случайности», передаётся симулятором из его генератора.
...
Первый шаг T-005 — замер на `--ops=1500` и `--ops=10000`. Если `sim:long` > 10 мин, заводится задача T-028: в `packages/crdt` — чистый `accumulate(deltas)`/накопитель с внутренним изменяемым `Map` и property-тестом `accumulate(ds) equals fold(merge)`. Решение — через скилл `crdt-op`, не в T-005.
