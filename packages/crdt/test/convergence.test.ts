...
describe("REQ-022: сходимость независимо от порядка и повторов доставки", () => {
  ...
  it("REQ-022: полный перебор всех перестановок для малого числа дельт (≤ 5) даёт одинаковое состояние", () => {
    // Исчерпывающий перебор внутри тела свойства — numRuns намеренно меньше
    // 100 (разрешённое исключение из инструкции): 5! = 120 перестановок на
    // каждый запуск уже даёт большое покрытие.
    fc.assert(
      fc.property(
        scenarioArb("exhaustive", { minOps: 1, maxOps: 5, minActors: 2, maxActors: 3 }),
        (scenario) => {
