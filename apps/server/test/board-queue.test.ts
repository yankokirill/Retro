...
// реализация станет реальным FIFO (docs/spec/consistency-model.md § 7 —
...
//
// Известно заранее (комментарий в самом board-queue.ts, T-012): текущая
// версия — сквозная (`run` вызывает fn немедленно, без упорядочивания),
// поэтому тесты на пункты 1 и 2(ordering под 3+ вызовами) должны падать.
